...
const MedicoDetails = () => {
  const [medicoDetails, setMedicoDetails] = useState<IMedicoDetails>();
  const {medicoId} = useParams();

  useEffect(() => {
    const url = `medicos/${medicoId}`;
    get(url).then(async (res: IMedicoDetails) => {
      setMedicoDetails(res);
    });
  }, [medicoId]);
  ...
  return <div className="providerContainer container">
    <div className="row ProviderDetails">
        <div className="col-lg-9">
            <div className="media">
                <img className="mr-3" src="/images/user.svg" alt="Medico" />
                <div className="media-body">
                    <h1 className="mt-0">{medicoDetails?.medico.firstName}, {medicoDetails?.medico.lastName}, {medicoDetails?.medico.designation}<span>{medicoDetails?.specialities.map(s => s.speciality).join(', ')}</span></h1>
                </div>
            </div>
        </div>
        ...
        <div className="col-sm-12">
            ...
            <div className="tab-content" id="myTabContent">
                ...
                <div className="tab-pane fade show active" id="servicesCost" role="tabpanel" aria-labelledby="profile-tab">
                                ...
                                {medicoDetails?.services.map(service => <div key={service.id} className="row costDetails">
